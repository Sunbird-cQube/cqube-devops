...
echo -e "\e[0;38m${bold}please enter the aws_access_key ${normal} \e[0;36m${bold}(Hint: AWS Access key for creation of s3 bucket)${normal}"
read aws_access_key
...
aws_access_key_id=$(awk ''/^aws_access_key:' /{ if ($2 !~ /#.*/) {print $2}}' config_files/aws_s3_config.yml)
aws_secret_access_key=$(awk ''/^aws_secret_key:' /{ if ($2 !~ /#.*/) {print $2}}' config_files/aws_s3_config.yml)
...
rm config_files/aws_s3_config.yml
touch config_files/aws_s3_config.yml
if [[ -e "config_files/aws_s3_config.yml" ]]; then
check_aws_key
check_aws_secret_key
check_archived_buc
fi
...
echo -e "\e[0;38m${bold} `cat config_files/aws_s3_config.yml` ${normal}"
echo -e "\e[0;33m${bold}Currently cQube aws_s3_config.yml is entered. Follow Installation process with above config values.${normal}"
echo -e "\e[0;33m${bold}If you want to edit config value please enter yes.${normal}"
            while true; do

             read -p "Do you still want to edit the aws_s3_config.yml file (yes/no)? " yn
             case $yn in
                 yes) break;;
                 no) break 2;;
                 * ) echo "Please answer yes or no.";;
