...
read az_name
azure_account_key=$(awk ''/^azure_account_key:' /{ if ($2 !~ /#.*/) {print $2}}' config_files/upgradation_azure_container_config.yml)
    az_account_status=0
...
rm config_files/upgradation_azure_container_config.yml
touch config_files/upgradation_azure_container_config.yml
if [[ -e "config_files/upgradation_azure_container_config.yml" ]]; then
check_az_storage_connection_string
check_az_key
check_az_storage_account_name
check_az_archived_container
...
echo -e "\e[0;38m${bold} `cat config_files/upgradation_azure_container_config.yml` ${normal}"
echo -e "\e[0;33m${bold}Currently cQube upgradation_azure_container_config.yml is entered. Follow Installation process with above config values.${normal}"
echo -e "\e[0;33m${bold}If you want to edit config value please enter yes.${normal}"
            while true; do

             read -p "Do you still want to edit the upgradation_azure_container_config.yml file (yes/no)? " yn
             case $yn in
                 yes) break;;
                 no) break 2;;
                 * ) echo "Please answer yes or no.";;
